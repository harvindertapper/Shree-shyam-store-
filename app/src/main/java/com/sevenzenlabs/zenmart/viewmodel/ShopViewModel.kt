...
import com.sevenzenlabs.zenmart.utils.AuthenticatedRestBackupProvider
import com.sevenzenlabs.zenmart.utils.BackupAuthContext
import com.sevenzenlabs.zenmart.utils.BackupIncompatibleException
import com.sevenzenlabs.zenmart.utils.BackupProviderException
import com.sevenzenlabs.zenmart.utils.BackupUnauthorizedException
import com.sevenzenlabs.zenmart.utils.CurrencyUtils
import com.sevenzenlabs.zenmart.utils.LocalLoginPolicy
import com.sevenzenlabs.zenmart.utils.LocalLoginResult
import com.sevenzenlabs.zenmart.utils.OperatorAction
import com.sevenzenlabs.zenmart.utils.OperatorActionPolicy
import com.sevenzenlabs.zenmart.utils.OperatorGateException
import com.sevenzenlabs.zenmart.utils.localizedOperatorGateMessage
import com.sevenzenlabs.zenmart.utils.PinUnlockResult
...
import com.sevenzenlabs.zenmart.utils.RestoreSnapshotException
...
import com.sevenzenlabs.zenmart.utils.SnapshotUnavailableException
...
class ShopViewModel(
    ...
    suspend fun reconcileIdentitySession(): IdentitySession? {
        ...
        val firebaseUser = com.sevenzenlabs.zenmart.utils.AuthManager.currentUser
    ...
    private fun markMutationSavedLocally() {
        ...
        _mutationStatus.value = MutationStatus(
            ...
            message = com.sevenzenlabs.zenmart.utils.LocaleHelper
        ...
        context?.let { ctx ->
            viewModelScope.launch {
                ...
                if (settings.isUserLoggedIn && settings.autoSyncEnabled) {
                    try {
                        com.sevenzenlabs.zenmart.utils.SyncManager.triggerImmediateSync(ctx)
                        com.sevenzenlabs.zenmart.utils.SyncManager.triggerAutomaticBackup(ctx)
                    } catch (_: Exception) {
    ...
    val storeSettings: StateFlow<StoreSettings> = settingsDataStore.settingsFlow
        .stateIn(
            ...
            initialValue = StoreSettings(
                ...
                appLanguage = com.sevenzenlabs.zenmart.utils.AppLanguage.HINDI
            )
        )
        ...
        viewModelScope.launch {
            try {
                ...
                context?.let { appContext ->
                    com.sevenzenlabs.zenmart.utils.SyncManager.configureAutomaticSync(
                        context = appContext,
                ...
                val message = com.sevenzenlabs.zenmart.utils.LocaleHelper
    ...
    fun recoverAppLockWithFirebase(
        ...
        viewModelScope.launch {
            try {
                ...
                val currentFirebaseUser = com.sevenzenlabs.zenmart.utils.AuthManager.currentUser
                ...
                val result = com.sevenzenlabs.zenmart.utils.AuthManager.signInWithGoogle(appContext)
    ...
    fun sendForgotPinEmail(
        ...
        viewModelScope.launch {
            ...
            val result = com.sevenzenlabs.zenmart.utils.AuthManager.sendPasswordResetEmail(email.trim(), context)
            if (result.isSuccess) {
                onSuccess()
            } else {
    ...
    fun logoutUser() {
        viewModelScope.launch {
            context?.let { ctx ->
                com.sevenzenlabs.zenmart.utils.AuthManager.signOut(ctx)
            }
            settingsDataStore.clearSession()
    ...
    fun generateInvoiceText(customSale: Sale? = null, customItems: List<SaleItem>? = null): String {
        ...
        return com.sevenzenlabs.zenmart.utils.ShareUtils.generateBillReceiptText(
    ...
    fun sendUdhaarReminder(context: Context, customer: Customer, balance: Long) {
        ...
        val strings = com.sevenzenlabs.zenmart.utils.LocaleHelper.getStrings(settings.appLanguage)
        val shopDisplayName = settings.shopName.ifEmpty { strings.defaultShopName }
        val msg = com.sevenzenlabs.zenmart.utils.ShareUtils.generateUdhaarReminderText(
            shopName = shopDisplayName,
    ...
    fun exportStockCsv(
        ...
        com.sevenzenlabs.zenmart.utils.ShareUtils.exportStockCsv(
            context = context,
            products = products,
    ...
    fun exportUdhaarCsv(context: Context, debtorCustomers: List<Customer>, balances: Map<Long, Long>) {
        ...
        val strings = com.sevenzenlabs.zenmart.utils.LocaleHelper.getStrings(settings.appLanguage)
        val shopDisplayName = settings.shopName.ifEmpty { strings.defaultShopName }
        com.sevenzenlabs.zenmart.utils.ShareUtils.exportUdhaarCsv(
            context = context,
    ...
    fun generateReorderText(lowStockList: List<Product>, categories: List<Category>): String {
        ...
        val shopDisplayName = settings.shopName.ifEmpty { strings.defaultShopName }
        val catMap = categories.associate { it.id to it.name }
        return com.sevenzenlabs.zenmart.utils.ShareUtils.generateReorderListText(
            shopName = shopDisplayName,
    ...
    private suspend fun authenticatedBackupClient(
        ...
        val url = settings.firebaseUrl.ifBlank { com.sevenzenlabs.zenmart.BuildConfig.FIREBASE_URL }.trim()
        val trustedHost = runCatching {
            URI(com.sevenzenlabs.zenmart.BuildConfig.FIREBASE_URL).host?.trim()?.lowercase()
        }.getOrNull()
    ...
    private fun backupFailureMessage(error: Throwable): String = when (error) {
        is BackupProviderException -> when (error) {
            ...
            is com.sevenzenlabs.zenmart.utils.BackupWrongTenantException -> "Backup belongs to a different store and was rejected."
            is com.sevenzenlabs.zenmart.utils.BackupUnavailableException -> "Backup provider is unavailable. Try again when online."
            is com.sevenzenlabs.zenmart.utils.BackupMalformedException -> "Backup data is malformed or incomplete. Local data was not changed."
            is com.sevenzenlabs.zenmart.utils.BackupIncompatibleException -> "Backup format or provider configuration is incompatible."
            is com.sevenzenlabs.zenmart.utils.BackupReplayException -> "This backup snapshot was already seen and was rejected."
        }
