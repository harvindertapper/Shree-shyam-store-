...
import com.sevenzenlabs.zenmart.utils.BackupHttpResponse
import com.sevenzenlabs.zenmart.utils.BackupMalformedException
import com.sevenzenlabs.zenmart.utils.BackupReplayException
import com.sevenzenlabs.zenmart.utils.BackupTransport
...
import com.sevenzenlabs.zenmart.utils.BackupWrongTenantException
import com.sevenzenlabs.zenmart.utils.CloudSyncPolicy
...
class AuthenticatedBackupProviderTest {
    ...
    fun unauthorizedResponseIsRejectedBeforeRestore() = runBlocking {
        ...
        assertThrows(BackupUnauthorizedException::class.java) {
            runBlocking { provider.download(request = com.sevenzenlabs.zenmart.utils.BackupDownloadRequest("categories")) }
                .also { roomWrites++ }
        }
    ...
    fun wrongTenantHeaderIsRejectedBeforeRestore() = runBlocking {
        ...
        assertThrows(BackupWrongTenantException::class.java) {
            runBlocking { provider.download(com.sevenzenlabs.zenmart.utils.BackupDownloadRequest("categories")) }
        }
        Unit
    }
    ...
    fun timeoutAndServerErrorAreUnavailable() = runBlocking {
        ...
        assertThrows(BackupUnavailableException::class.java) {
            runBlocking { serverErrorProvider.download(com.sevenzenlabs.zenmart.utils.BackupDownloadRequest("categories")) }
        }

        val timeoutProvider = provider { throw SocketTimeoutException("timed out") }
        assertThrows(BackupUnavailableException::class.java) {
            runBlocking { timeoutProvider.download(com.sevenzenlabs.zenmart.utils.BackupDownloadRequest("categories")) }
        }
        Unit
    }
    ...
    fun replayedSnapshotIsRejected() = runBlocking {
        ...
        val first = provider.download(
            com.sevenzenlabs.zenmart.utils.BackupDownloadRequest("categories")
        )
        ...
        assertThrows(BackupReplayException::class.java) {
            runBlocking {
                provider.download(
                    com.sevenzenlabs.zenmart.utils.BackupDownloadRequest(
    ...
    fun credentialTablesAreExcludedFromAllowlist() = runBlocking {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            runBlocking {
                provider.upload(
                    com.sevenzenlabs.zenmart.utils.BackupUploadRequest(
                        tableName = "users",
